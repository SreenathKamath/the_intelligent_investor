...
const fallbackIndices = [
  {
    ...
    series: [23100, 23240, 23310, 23280, 23460, 23600, 23720, 23810, 23950, 24218],
  },
  {
    symbol: "^BSESN",
    name: "Sensex",
    ...
    rangeLabel: "Fallback data",
    updatedAt: "offline mode",
    series: [79020, 79150, 79210, 79380, 79520, 79680, 79740, 79820, 80020, 79944],
  },
  {
    symbol: "^NSEBANK",
    name: "Bank Nifty",
    ...
    rangeLabel: "Fallback data",
    updatedAt: "offline mode",
    series: [50120, 50250, 50320, 50480, 50520, 50710, 50860, 50990, 51060, 51102],
  },
];
...
const sectorMap = {
  banking: {
    ...
    stocks: [
      { symbol: "HDFCBANK.NS", weight: 29.0 },
      { symbol: "ICICIBANK.NS", weight: 24.0 },
      { symbol: "AXISBANK.NS", weight: 10.0 },
      { symbol: "SBIN.NS", weight: 9.0 },
      { symbol: "KOTAKBANK.NS", weight: 8.0 },
      { symbol: "BANKBARODA.NS", weight: 4.0 },
      { symbol: "INDUSINDBK.NS", weight: 4.0 },
      { symbol: "PNB.NS", weight: 3.0 },
      { symbol: "FEDERALBNK.NS", weight: 2.0 },
      { symbol: "AUBANK.NS", weight: 2.0 },
    ],
  },
  it: {
    key: "it",
    ...
    stocks: [
      { symbol: "TCS.NS", weight: 28.0 },
      { symbol: "INFY.NS", weight: 24.0 },
      { symbol: "HCLTECH.NS", weight: 11.0 },
      { symbol: "WIPRO.NS", weight: 9.0 },
      { symbol: "TECHM.NS", weight: 8.0 },
      { symbol: "LTIM.NS", weight: 7.0 },
      { symbol: "PERSISTENT.NS", weight: 4.0 },
      { symbol: "MPHASIS.NS", weight: 3.0 },
      { symbol: "COFORGE.NS", weight: 3.0 },
      { symbol: "OFSS.NS", weight: 1.0 },
    ],
  },
  pharma: {
    key: "pharma",
    ...
    stocks: [
      { symbol: "SUNPHARMA.NS", weight: 26.0 },
      { symbol: "DIVISLAB.NS", weight: 13.0 },
      { symbol: "CIPLA.NS", weight: 12.0 },
      { symbol: "DRREDDY.NS", weight: 11.0 },
      { symbol: "LUPIN.NS", weight: 8.0 },
      { symbol: "TORNTPHARM.NS", weight: 7.0 },
      { symbol: "ZYDUSLIFE.NS", weight: 6.0 },
      { symbol: "ALKEM.NS", weight: 5.0 },
      { symbol: "BIOCON.NS", weight: 4.0 },
      { symbol: "ABBOTINDIA.NS", weight: 3.0 },
    ],
  },
  fmcg: {
    key: "fmcg",
    ...
    stocks: [
      { symbol: "HINDUNILVR.NS", weight: 30.0 },
      { symbol: "ITC.NS", weight: 18.0 },
      { symbol: "NESTLEIND.NS", weight: 14.0 },
      { symbol: "VBL.NS", weight: 8.0 },
      { symbol: "TATACONSUM.NS", weight: 7.0 },
      { symbol: "BRITANNIA.NS", weight: 6.0 },
      { symbol: "DABUR.NS", weight: 5.0 },
      { symbol: "GODREJCP.NS", weight: 4.0 },
      { symbol: "COLPAL.NS", weight: 4.0 },
      { symbol: "MARICO.NS", weight: 4.0 },
    ],
  },
  energy: {
    key: "energy",
    ...
    stocks: [
      { symbol: "RELIANCE.NS", weight: 38.0 },
      { symbol: "ONGC.NS", weight: 11.0 },
      { symbol: "IOC.NS", weight: 10.0 },
      { symbol: "BPCL.NS", weight: 8.0 },
      { symbol: "GAIL.NS", weight: 7.0 },
      { symbol: "HINDPETRO.NS", weight: 6.0 },
      { symbol: "ADANIPOWER.NS", weight: 6.0 },
      { symbol: "POWERGRID.NS", weight: 5.0 },
      { symbol: "NTPC.NS", weight: 5.0 },
      { symbol: "OIL.NS", weight: 4.0 },
    ],
  },
};
...
function isIndianQuote(item) {
  ...
  return (
    ...
    exchange.includes("NATIONAL STOCK EXCHANGE OF INDIA") ||
...
async function fetchYahooIndex(symbol, config) {
  ...
  if (!meta || !closes.length) {
    throw new Error(`Invalid Yahoo payload for ${symbol}`);
  }
  ...
  return {
    symbol,
    name: symbolMap.find((item) => item.symbol === symbol)?.name || symbol,
    price,
    changePercent,
...
async function fetchYahooChartSnapshot(symbol) {
  const config = rangeConfig["1y"];
  const data = await fetchYahooIndex(symbol, config);
  return {
    price: data.price,
...
async function fetchYahooQuoteBatch(symbols) {
  ...
  if (!response.ok) {
    throw new Error("Yahoo quote batch failed");
  }
...
async function fetchYahooQuoteSummary(symbol) {
  ...
  if (!response.ok) {
    throw new Error("Yahoo quote summary failed");
  }
...
async function resolveIndianSymbol(queryOrSymbol) {
  ...
  if (indianSymbolOverrides[raw]) {
    return indianSymbolOverrides[raw];
  }
  ...
  const sectorMatch = Object.values(sectorMap)
    .flatMap((sector) => sector.stocks.map((stock) => stock.symbol))
    .find((symbol) => symbol.toUpperCase() === `${raw}.NS` || symbol.toUpperCase().startsWith(`${raw}.NS`));
  ...
  try {
    ...
    if (directIndian?.symbol) {
      return directIndian.symbol;
    }
  } catch (error) {
  ...
  try {
    ...
    const bestIndian = (payload?.quotes || [])
      .filter((item) => item.symbol)
      .sort((a, b) => searchScore(b, raw) - searchScore(a, raw))[0];

    if (bestIndian?.symbol) {
      return bestIndian.symbol;
    }
  } catch (error) {
    // Fallback below.
  }
...
async function handleIndices(response, requestUrl) {
...
async function handleScreenerSectors(response) {
  ...
  try {
    ...
    sendJson(response, 200, { sectors: liveSectors });
  } catch (error) {
    sendJson(response, 200, {
...
async function handleScreenerSector(response, requestUrl) {
...
async function handleScreenerSearch(response, requestUrl) {
  ...
  try {
    ...
    sendJson(response, 200, { bestMatch, matches });
  } catch (error) {
    const resolvedSymbol = await resolveIndianSymbol(query);
    sendJson(response, 200, {
...
async function handleScreenerStock(response, requestUrl) {
  const requestedSymbol = requestUrl.searchParams.get("symbol");
  if (!requestedSymbol) {
    sendJson(response, 400, { error: "Missing symbol" });
    return;
  }
